...
// EmailJS konfiguracija (zameni po potrebi svojim vrednostima)
...
// Fiksni primalac narudžbina
const RECIPIENT_EMAIL = "stellamicrogreens@gmail.com";

// Fallback slika ako neke nedostaju
...
export default function App() {
  ...
  const dodajUkorpu = (proizvod) => {
    ...
    // skrol do korpe
    setTimeout(() => narudzbinaRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 60);
  };

  const ukloniProizvod = (index) => setPorudzbina((prev) => prev.filter((_, i) => i !== index));
          ...
          {proizvodi.map((p, i) => (
            <div key={i} className="border border-green-100 rounded-2xl bg-white shadow p-4">
              <img
                src={`/images/${p.slika}`}
                alt={p.ime}
                onError={(e) => { e.currentTarget.src = PLACEHOLDER_IMG; e.currentTarget.onerror = null; }}
            ...
            <ul className="list-disc pl-4 space-y-1">
              {porudzbina.map((item, i) => (
                <li key={`${item.ime}-${item.gramaza}-${i}`} className="flex items-center justify-between">
                  ...
                  <button onClick={() => ukloniProizvod(i)} className="text-red-600 text-xs hover:underline">Ukloni</button>
                </li>
              ))}
            </ul>
...
// ——— Inline testovi (u dev okruženju) ———
